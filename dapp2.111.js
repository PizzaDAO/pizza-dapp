...
const BOX_ADDRESS = "0x9600ac15298b7fe99addf8844cb5492da0d0d33a";
...
const BOX_ABI = [
	...
	{
		inputs: [
			{ internalType: "address", name: "chainlinkBTCETHFeed", type: "address" },
		],
		name: "initialize",
		outputs: [],
		stateMutability: "nonpayable",
	...
	},
	{
		inputs: [
			{ internalType: "uint8", name: "count", type: "uint8" },
			{ internalType: "address[]", name: "toPaisanos", type: "address[]" },
		],
...
const PIZZA_ABI = [
	...
	{
		inputs: [
			{ internalType: "address", name: "orderAPIClient", type: "address" },
		],
...
const onLoadHandler = () => {
	...
	const triggerPurchase = () => {
		...
		if (mainSaleActive) {
          ...
          <a href='https://${NETWORK}etherscan.io/tx/${txHash}' target='_blank'> Transaction link </a> </p>`;
			...
			BoxInstance.methods
				.prePurchase(proof, numberToMint)
				.send({
					from: walletAddress,
					value: priceInWei * numberToMint,
				})
	...
	const createPrePurchaseProof = (indexOfUser) => {
		...
		const claimListMerkleTree = new MerkleTree(
			claimListHashes,
			keccak256.keccak256,
			{ sort: true }
		);
	...
	const getMerkleRoot = (claimList) => {
		...
		const claimListMerkleTree = new MerkleTree(leaves, keccak256, {
			sort: true,
		});
		console.log("root", claimListMerkleTree.getHexRoot());
		return {
			tree: claimListMerkleTree,
			root: "0x" + claimListMerkleTree.getHexRoot(),
			data: data,
		};
	};
	...
	const getPresaleProof = (claimList, walletAddress) => {
		...
		console.log(
			claimListMerkleTree.getHexProof(
				ethers.utils.solidityKeccak256(["address"], [walletAddress])
			)
		);
		return claimListMerkleTree.getHexProof(
			ethers.utils.solidityKeccak256(["address"], [walletAddress])
		);
	...
	const proofmtjs = (claimList) => {
		...
		const leaves = claimList.map((x) =>
			ethers.utils.solidityKeccak256(x.toLowerCase())
		);
