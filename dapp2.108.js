...
const onLoadHandler = () => {
      ...
      BoxInstance.methods.balanceOf(walletAddress).call()
        .then(async (balance) => {
          ...
          const resultsUnclaimed = await Promise.all(
            boxes.map(boxId =>
              PizzaInstance.methods.isClaimed(boxId).call().then(value => !value)
            )
          )
          console.log("resultsUnclaimed: ", resultsUnclaimed)

          // Boxes to bake for pizzas
          ...
          // Clear out select boxes before we fill them again
          ...
          while (claimSelector.options.length > 1) {
            // if it's the default select option, return
            if (!claimSelector.options[0].value) {
              return
            }

            claimSelector.remove(0)
          }

          // Ordering and populating select boxes
  ...
  const claimButtonHandler = async () => {
    hide(claimButton)
    console.log('Bake pie button pressed')
    if (claimSelector.options.length > 1) {
      await handleUser()
      if (!addresses.length) {
        console.log("prompting metamask")
        promptMetamask()
        ...
        PizzaInstance.methods.claim(parseFloat(claimSelector.value)).send({from:walletAddress})
          .on('transactionHash', (hash) => {
            console.log('transactionHash: ', hash)
            txHash = hash
            display(claimButton)
  ...
  claimButton.addEventListener('click', claimButtonHandler)
